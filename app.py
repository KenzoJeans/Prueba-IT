...
def cargar_historico_firmas():
    ...
    try:
        ...
        # Si la hoja no existe, retorna DataFrame vacío
...
def guardar_firma(placa, usuario, conformidad, observaciones, firma_base64):
    """
    Guarda la firma en un JSON local (Streamlit Cloud).
    En producción, enviar a Google Sheets con API.
    """
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    
    # Para este ejemplo, usamos st.session_state como histórico temporal
...
def obtener_firmas_por_equipo(placa):
    """Retorna todas las firmas asociadas a una placa"""
    if 'firmas_guardadas' not in st.session_state:
        st.session_state.firmas_guardadas = []
    
    firmas_equipo = [f for f in st.session_state.firmas_guardadas if f['PLACA'] == placa]
    return firmas_equipo
...
def calcular_sla_compliance(df):
    """Calcula el cumplimiento de SLA basado en equipos en estado 'Operativo'"""
...
def obtener_detalle_equipo(registro, col_placas):
    """Formatea un dict con todos los datos relevantes del equipo"""
    detalle = {}
...
tab_dashboard, tab_firma, tab_datos, tab_firmas = st.tabs(["📊 Dashboard de Gestión", "✍️ Validación y Firma de Actas", "📋 Datos Completos", "📄 Historial de Firmas"])
...
with tab_dashboard:
    ...
    if not df_mantenimiento.empty:
        ...
        cols_mostrar = [c for c in df_mantenimiento.columns if c not in ['FECHA_CLEAN', 'DISPLAY_NAME']][:6]
...
with tab_firmas:
    ...
    if 'firmas_guardadas' in st.session_state and st.session_state.firmas_guardadas:
        # Convertir historial a DataFrame para visualización
        ...
        # Mostrar firmas agrupadas por placa
...
def cargar_datos_mantenimiento():
    ...
    try:
        ...
        # Crear versión UPPER para búsqueda, pero guardar originales
        df_upper = df.copy()
        df_upper.columns = df_upper.columns.str.upper()
        ...
        # Pasar las columnas uppercase al dataframe original para búsqueda
        df.columns = df_upper.columns
...
tab_dashboard, tab_firma, tab_datos = st.tabs(["📊 Dashboard de Gestión", "✍️ Validación y Firma de Actas", "📋 Datos Completos"])
...
with tab_dashboard:
    ...
    if not df_mantenimiento.empty:
        ...
        # KPI de SLA con color condicional
        sla_color = "#4ade80" if sla_compliance >= 95 else "#f59e0b" if sla_compliance >= 80 else "#ef4444"
        col4.markdown(f"<div class='kpi-card'><div class='kpi-label'>Cumplimiento SLA</div><div class='kpi-value' style='color:{sla_color};'>{sla_compliance}%</div></div>", unsafe_allow_html=True)
        ...
        # Nuevo: tabla de resumen reciente
        st.write("---")
        st.markdown("### 📅 Registros Más Recientes")
        cols_mostrar = [c for c in df_mantenimiento.columns if c not in ['FECHA_CLEAN', 'DISPLAY_NAME']][:6]
        df_resumen = df_mantenimiento.drop(columns=['FECHA_CLEAN', 'DISPLAY_NAME'], errors='ignore').head(10)
...
with tab_firma:
    ...
    if not df_mantenimiento.empty:
        ...
        if col_placas and col_usuario:
            # Crear selector mejorado con filtros
            st.markdown("#### 🔍 Búsqueda del Equipo")
            col_busqueda_placa, col_busqueda_usuario = st.columns(2)
            ...
            if not registro_actual.empty:
                ...
                # Mostrar tabla detallada del equipo
                st.write("---")
                st.markdown("### 📋 Detalles del Mantenimiento")
                ...
                # Histórico del equipo (si hay mantenimientos previos)
                historico_equipo = df_mantenimiento_full[df_mantenimiento_full[col_placas] == placa_selec]
                if len(historico_equipo) > 1:
                    st.write("---")
                    st.markdown("### 📊 Histórico de Mantenimientos para Este Equipo")
                ...
                if firma_canvas.image_data is not None:
                    if st.button("💾 Guardar Validación de Mantenimiento", type="primary"):
                        # Aquí va la lógica para guardar la firma (ejemplo: a Google Sheets, email, etc.)
                        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        st.success(
                            f"""
                            ✅ **Firma capturada exitosamente**
                            ...
                            El acta ha sido registrada en el sistema.
                            """
                        )
                        st.balloons()
                else:
